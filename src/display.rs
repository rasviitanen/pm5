...
mod tests {
    ...
    struct MockCsafe;
    impl CsafeInterface for MockCsafe {
        fn send_packet(&mut self, packet: &[u8]) -> Result<()> {
            println!("Sending packet: {} bytes", packet.len());
            Ok(())
        }
    }

    #[test]
    fn test_bitmap_display() -> Result<()> {
        let mut screen = Pm5Bitmap::new();

        // Draw a simple leaderboard
        screen.draw_text(10, 5, "LEADERBOARD");
        screen.draw_text(10, 20, "1. RUSTACEAN  1:45");
        screen.draw_text(10, 30, "2. PYTHONISTA 1:50");
        screen.draw_text(10, 40, "3. GOPHER     1:52");

        // Add a border
        screen.fill_rect(0, 0, SCREEN_WIDTH, 1, true); // Top
        screen.fill_rect(0, SCREEN_HEIGHT - 1, SCREEN_WIDTH, 1, true); // Bottom

        let packets = screen.to_csafe_packets();
        println!("Generated {} packets for 240x64 screen", packets.len());

        // Mock sending
        let mut csafe = MockCsafe;
        display_bitmap(&screen, &mut csafe)?;
